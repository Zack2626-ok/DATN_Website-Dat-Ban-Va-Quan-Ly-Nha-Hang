...
export const initDb = async (): Promise<boolean> => {
  ...
  // Seed a default admin/manager user when users table is empty
  try {
    const userCount = await query<any[]>("SELECT COUNT(*) as count FROM users");
    if (userCount[0].count === 0) {
      const defaultPasswordHash = await bcrypt.hash("123456", 10);
      await query(`
        INSERT INTO users (role_id, employee_code, full_name, email, password_hash, phone, status)
        VALUES
          (1, 'NV001', 'System Admin', 'admin@gmail.com', ?, '0900000001', 'active'),
          (2, 'NV002', 'Restaurant Manager', 'manager@gmail.com', ?, '0900000002', 'active')
      `, [defaultPasswordHash, defaultPasswordHash]);
      console.log("✅ Seeded default admin and manager users.");
    }
  } catch (err: any) {
    console.warn("Seeding default users skipped:", err.message);
  }
