...
export const createBooking = async (data: any): Promise<any> => {
  ...
  if (data.guest_phone || validCustomerId) {
    const customerOverlaps = await query<any[]>(`
  ...
  // Tự động kiểm tra và thêm cột guest_email vào bảng bookings nếu MySQL chưa có
  try {
    ...
    if (colCheck.length === 0) {
      await query(`ALTER TABLE bookings ADD COLUMN guest_email VARCHAR(150) DEFAULT NULL AFTER guest_phone`);
      console.log("✅ Auto-migration: Đã tự động thêm cột guest_email vào bảng bookings");
    }
  } catch (colErr) {
    console.warn("Auto-migration guest_email check:", (colErr as Error).message);
  }
