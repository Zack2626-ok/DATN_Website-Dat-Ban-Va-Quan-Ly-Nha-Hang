...
// ============================================================================
//  RESMANAGER SCHEMA — Analytics & Reports (Module 7)
// ============================================================================

export interface DbRevenueOverview {
  date: string;
  revenue: number;
  orderCount: number;
}

export interface DbPaymentMethodStat {
  method: string;
  total: number;
  count: number;
}

export interface DbTopItem {
  id: number;
  name: string;
  quantity: number;
  revenue: number;
...
export const getDbRevenueOverview = async (startDate?: string, endDate?: string): Promise<DbRevenueOverview[]> => {
  if (!dbAvailable) {
    return [
      { date: "2026-07-10", revenue: 5000000, orderCount: 12 },
      { date: "2026-07-11", revenue: 7500000, orderCount: 15 },
      { date: "2026-07-12", revenue: 6200000, orderCount: 10 },
      { date: "2026-07-13", revenue: 9000000, orderCount: 22 },
      { date: "2026-07-14", revenue: 4500000, orderCount: 9 },
      { date: "2026-07-15", revenue: 11000000, orderCount: 25 },
...
export const getDbPaymentMethods = async (startDate?: string, endDate?: string): Promise<DbPaymentMethodStat[]> => {
  if (!dbAvailable) {
    return [
      { method: "cash", total: 15000000, count: 40 },
      { method: "transfer", total: 20000000, count: 35 },
      { method: "card", total: 8000000, count: 15 },
...
export const getDbTopItems = async (startDate?: string, endDate?: string): Promise<DbTopItem[]> => {
  if (!dbAvailable) {
    return [
      { id: 1, name: "Cá hồi sốt chanh bơ", quantity: 33, revenue: 7260000 },
      { id: 2, name: "Bò lúc lắc khoai tây", quantity: 28, revenue: 5040000 },
      { id: 3, name: "Coca Cola lon", quantity: 22, revenue: 440000 },
      { id: 4, name: "Chả giò tôm thịt", quantity: 21, revenue: 1680000 },
      { id: 5, name: "Gà nướng mật ong", quantity: 19, revenue: 3040000 }
    ];
  }
  const conditions: string[] = ["o.status = 'completed'", "oi.status NOT IN ('cancelled', 'voided')"];
  const params: any[] = [];
  if (startDate) {
...
export const getDbCashFlow = async (startDate?: string, endDate?: string): Promise<{ income: number; expenses: number; netProfit: number; expenseItems: any[] }> => {
  if (!dbAvailable) {
    return {
      income: 35000000,
      expenses: 12000000,
      netProfit: 23000000,
      expenseItems: [
        { category: "Thịt & hải sản", amount: 8000000 },
        { category: "Rau củ quả", amount: 4000000 }
      ]
    };
  }
  const { dateCol } = await resolvePaymentColumns();
  ...
  const paymentRows = await query("SELECT SUM(amount) AS total FROM payments WHERE " + whereClause, params);
  const retailIncome = Number(paymentRows[0]?.total || 0);

  const eventConditions: string[] = ["status IN ('confirmed', 'completed')"];
  const eventParams: any[] = [];
  if (startDate) {
    eventConditions.push("event_date >= ?");
  ...
  const eventWhereClause = eventConditions.join(" AND ");
  const eventRows = await query("SELECT SUM(total_amount) AS total FROM event_contracts WHERE " + eventWhereClause, eventParams);
  const eventIncome = Number(eventRows[0]?.total || 0);

  const income = retailIncome + eventIncome;

  let expenses = 0;
  const expenseItems: any[] = [];
  try {
    const stockConditions: string[] = ["transaction_type = 'stock_in'"];
    const stockParams: any[] = [];
    if (startDate) {
      stockConditions.push("created_at >= ?");
    ...
    const stockWhere = stockConditions.join(" AND ");
    const stockRows = await query("SELECT SUM(quantity * unit_price) AS total FROM inventory_transactions WHERE " + stockWhere, stockParams);
    expenses = Number(stockRows[0]?.total || 0);
    if (expenses > 0) {
      expenseItems.push({ category: "Nguyên liệu nhập kho", amount: expenses });
    }
  } catch (e) {
    expenses = 0;
  }
...
export const getDbPeakHours = async (startDate?: string, endDate?: string): Promise<{ hour: number; count: number; percentage: number }[]> => {
  if (!dbAvailable) {
    return Array.from({ length: 15 }, (_, i) => ({
      hour: 8 + i,
      count: i === 4 || i === 11 ? 15 : 4,
      percentage: i === 4 || i === 11 ? 25 : 6
    }));
  }
  const conditions: string[] = ["status = 'completed'"];
  const params: any[] = [];
