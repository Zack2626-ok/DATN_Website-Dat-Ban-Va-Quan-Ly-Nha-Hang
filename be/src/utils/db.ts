...
export const query = async <T = any>(
  ...
  if (!connectionPool && !isInitializing) {
    ...
    try {
      console.log(
        "Database connection pool is not initialized. Attempting auto-initialization...",
      );
      await initDb();
    } catch (err) {
...
const normalizePayment = (row: any): Payment => ({
  ...
  discountAmount:
    row.discountAmount !== null && row.discountAmount !== undefined
...
const normalizeOrder = (row: any): Order => {
  ...
  try {
    ...
    console.error(
      "Lỗi khi phân tích JSON items cho order:",
      row.id,
      row.items,
      err,
    );
...
export const initDb = async (): Promise<boolean> => {
  ...
  try {
    await query(
      "UPDATE payments SET amount = amount / 1000 WHERE amount > 100000000",
    );
    await query(
      "UPDATE invoices SET subtotal = subtotal / 1000, tax = tax / 1000, discount = discount / 1000, total = total / 1000 WHERE total > 100000000",
    );
    console.log(
      "✅ Đã chuẩn hóa số tiền các thanh toán/hóa đơn lịch sử (> 100 triệu) về đơn vị gốc.",
    );
  } catch (errCleanup: any) {
    console.warn(
      "⚠️ Không thể tự động dọn dẹp số tiền lịch sử:",
      errCleanup.message,
    );
  }
...
const runSchemaMigrations = async (): Promise<void> => {
  try {
    ...
    if (cols.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN is_held TINYINT(1) NOT NULL DEFAULT 0 AFTER status`,
      );
    ...
    if (statusCol.length > 0 && !statusCol[0].COLUMN_TYPE.includes("served")) {
      ...
      `);
      console.log(
        "✅ Migration: updated order_items.status ENUM to include served and delivered",
      );
    }
    ...
    if (colsDismissed.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN chef_dismissed TINYINT(1) NOT NULL DEFAULT 0 AFTER is_held`,
      );
    ...
    if (colsCreatedBy.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN created_by VARCHAR(50) DEFAULT NULL`,
      );
      await query(
        `UPDATE order_items oi JOIN orders o ON oi.order_id = o.id SET oi.created_by = o.created_by WHERE oi.created_by IS NULL`,
      );
    ...
    if (empCols.length === 0) {
      await query(
        `ALTER TABLE users ADD COLUMN employee_code VARCHAR(20) DEFAULT NULL AFTER role_id`,
      );
      await query(
        `UPDATE users SET employee_code = CONCAT('NV', LPAD(id, 3, '0')) WHERE employee_code IS NULL`,
      );
    ...
    await query(
      `ALTER TABLE tables MODIFY COLUMN status ENUM('empty','reserved','serving','pending_payment','cleaning','maintenance') NOT NULL DEFAULT 'empty'`,
    );
    ...
    if (guestCountCols.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN guest_count INT DEFAULT NULL`,
      ).catch(() => {});
    ...
    if (guestNameCols.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN guest_name VARCHAR(100) DEFAULT NULL`,
      ).catch(() => {});
    ...
    if (guestPhoneCols.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN guest_phone VARCHAR(20) DEFAULT NULL`,
      ).catch(() => {});
    ...
    if (orderBookingIdColumn.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN booking_id INT NULL AFTER table_id`,
      );
      await query(
        `ALTER TABLE orders ADD INDEX idx_orders_booking_id (booking_id)`,
      );
    ...
    if (mergedIntoTableColumn.length === 0) {
      await query(
        `ALTER TABLE tables ADD COLUMN merged_into_table_id INT NULL`,
      ).catch(() => {});
      await query(
        `ALTER TABLE tables ADD INDEX idx_tables_merged_into (merged_into_table_id)`,
      ).catch(() => {});
    ...
    if (mergedIntoOrderColumn.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN merged_into_order_id INT NULL`,
      ).catch(() => {});
      await query(
        `ALTER TABLE orders ADD INDEX idx_orders_merged_into (merged_into_order_id)`,
      ).catch(() => {});
    ...
    try {
      ...
      if (
        orderStatusColumn.length > 0 &&
        !String(orderStatusColumn[0].COLUMN_TYPE).includes(ORDER_STATUS.MERGED)
      ) {
        await query(
          `ALTER TABLE orders MODIFY COLUMN status VARCHAR(50) NOT NULL DEFAULT 'open'`,
        ).catch(() => {});
    ...
    if (earlyPaymentColumn.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN is_early_payment TINYINT(1) NOT NULL DEFAULT 0 AFTER status`,
      );
      await query(
        `ALTER TABLE orders ADD COLUMN is_early_paid TINYINT(1) NOT NULL DEFAULT 0 AFTER is_early_payment`,
      );
      console.log(
        "Migration: added orders.is_early_payment and orders.is_early_paid",
      );
    }
    ...
    if (sourceOrderColumn.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN source_order_id INT NULL AFTER order_id`,
      );
      await query(
        `ALTER TABLE order_items ADD INDEX idx_order_items_source_order (source_order_id)`,
      );
    ...
    if (colsIsCookedCancelled.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN is_cooked_cancelled TINYINT(1) NOT NULL DEFAULT 0`,
      );
    ...
    if (colsWasReused.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN was_reused TINYINT(1) NOT NULL DEFAULT 0`,
      );
    ...
    if (colsReusedBy.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN reused_by_order_item_id INT DEFAULT NULL`,
      );
    ...
    if (colsIsReused.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN is_reused TINYINT(1) NOT NULL DEFAULT 0`,
      );
    ...
    if (mergeTableExists.length === 0) {
      ...
      const mergeColumnNames = new Set(
        mergeColumns.map((column) => String(column.COLUMN_NAME)),
      );
      if (!mergeColumnNames.has("primary_order_id"))
        await query(
          `ALTER TABLE table_merges ADD COLUMN primary_order_id INT NULL AFTER merged_table_id`,
        );
      if (!mergeColumnNames.has("merged_order_id"))
        await query(
          `ALTER TABLE table_merges ADD COLUMN merged_order_id INT NULL AFTER primary_order_id`,
        );
      if (!mergeColumnNames.has("merged_by"))
        await query(
          `ALTER TABLE table_merges ADD COLUMN merged_by INT NULL AFTER merged_order_id`,
        );
      if (!mergeColumnNames.has("status"))
        await query(
          `ALTER TABLE table_merges ADD COLUMN status ENUM('active','resolved') NOT NULL DEFAULT 'active' AFTER merged_by`,
        );
      if (!mergeColumnNames.has("resolved_at"))
        await query(
          `ALTER TABLE table_merges ADD COLUMN resolved_at DATETIME NULL AFTER merged_at`,
        );
    }
    ...
    const mergeConstraintNames = new Set(
      mergeConstraints.map((constraint) => String(constraint.CONSTRAINT_NAME)),
    );
    ...
    const existingBookingCols = new Set(
      bookingCols.map((c) => String(c.COLUMN_NAME).toLowerCase()),
    );

    if (!existingBookingCols.has("pre_order_total")) {
      await query(
        `ALTER TABLE bookings ADD COLUMN pre_order_total DECIMAL(12,2) NOT NULL DEFAULT 0.00`,
      ).catch(() => {});
      console.log("✅ Migration: added bookings.pre_order_total");
    }
    if (!existingBookingCols.has("deposit_amount")) {
      await query(
        `ALTER TABLE bookings ADD COLUMN deposit_amount DECIMAL(12,2) NOT NULL DEFAULT 0.00`,
      ).catch(() => {});
      console.log("✅ Migration: added bookings.deposit_amount");
    }
    if (!existingBookingCols.has("deposit_status")) {
      await query(
        `ALTER TABLE bookings ADD COLUMN deposit_status ENUM('none', 'unpaid', 'paid', 'refunded', 'completed') NOT NULL DEFAULT 'none'`,
      ).catch(() => {});
    ...
    await query(
      `ALTER TABLE bookings MODIFY COLUMN status ENUM('pending','confirmed','arrived','completed','cancelled') NOT NULL DEFAULT 'pending'`,
    ).catch(() => {});
    console.log(
      "✅ Migration: updated bookings.status ENUM to include arrived",
    );
    ...
    // Migration: Tạo bảng notifications nếu chưa có
    await query(`
      CREATE TABLE IF NOT EXISTS notifications (
        id INT AUTO_INCREMENT PRIMARY KEY,
        title VARCHAR(255) NOT NULL,
    ...
    await query(
      "ALTER TABLE orders MODIFY COLUMN order_type VARCHAR(50) NOT NULL DEFAULT 'dine_in'",
    ).catch(() => {});
    await query(
      "ALTER TABLE order_items MODIFY COLUMN status VARCHAR(50) NOT NULL DEFAULT 'pending'",
    ).catch(() => {});
    ...
    if (voucherCostCol.length === 0) {
      await query(
        `ALTER TABLE vouchers ADD COLUMN points_cost INT NOT NULL DEFAULT 0 AFTER value`,
      );
      await query(
        `UPDATE vouchers SET points_cost = 100 WHERE code = 'SAVE10'`,
      );
      await query(
        `UPDATE vouchers SET points_cost = 300 WHERE code = 'FIXED50'`,
      );
      await query(`UPDATE vouchers SET points_cost = 200 WHERE code = 'NEW20'`);
      await query(
        `UPDATE vouchers SET points_cost = 150 WHERE code = 'SILVER15'`,
      );
      await query(
        `UPDATE vouchers SET points_cost = 250 WHERE code = 'GOLD25'`,
      );
    ...
    if (voucherTierCol.length === 0) {
      await query(
        `ALTER TABLE vouchers ADD COLUMN required_member_level VARCHAR(20) NULL AFTER points_cost`,
      );
      console.log(
        "✅ Migration: added required_member_level column to vouchers table",
      );
    }

    for (const reward of TIER_REWARD_VOUCHERS) {
      const existingReward = await query<TableIdRow[]>(
    ...
    if (stockInCols.length === 0) {
      ...
      `);
      await query(
        `UPDATE stock_in SET remaining_quantity = quantity WHERE remaining_quantity = 0`,
      );
      console.log(
        "✅ Migration: added batch_code, expiry_date, remaining_quantity to stock_in table",
      );
    }
    ...
    if (recreateSplits) {
      ...
      `);
      console.log(
        "✅ Migration: created/recreated table_splits table with correct schema",
      );
    }
    ...
    await query(
      `ALTER TABLE debt_payments ADD COLUMN proof_image LONGTEXT NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE debt_payments ADD COLUMN remaining_debt DECIMAL(14,2) NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE debt_payments ADD COLUMN is_deleted TINYINT(1) DEFAULT 0`,
    ).catch(() => {});
    await query(
      `ALTER TABLE debt_payments MODIFY COLUMN paid_by INT NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE stock_in ADD COLUMN is_credit TINYINT(1) DEFAULT 0`,
    ).catch(() => {});
    await query(`ALTER TABLE stock_in ADD COLUMN due_date DATE NULL`).catch(
      () => {},
    );
    await query(
      `ALTER TABLE stock_in ADD COLUMN paid_amount DECIMAL(14,2) NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE stock_in ADD COLUMN proof_image LONGTEXT NULL`,
    ).catch(() => {});
    ...
    await query(
      `ALTER TABLE operational_expenses ADD COLUMN created_by INT NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE operational_expenses ADD COLUMN is_recurring TINYINT(1) NOT NULL DEFAULT 0`,
    ).catch(() => {});
    await query(
      `ALTER TABLE operational_expenses ADD COLUMN deleted_at DATETIME NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE operational_expenses ADD COLUMN deleted_by INT NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE operational_expenses ADD COLUMN deleted_reason VARCHAR(255) NULL`,
    ).catch(() => {});
    ...
    if (hourlyCols.length === 0) {
      await query(
        `ALTER TABLE users ADD COLUMN hourly_rate DECIMAL(15, 2) NOT NULL DEFAULT 25000`,
      ).catch(() => {});
    ...
    if (dobCols.length === 0) {
      ...
      await query(
        `UPDATE users SET date_of_birth = '1998-08-18' WHERE date_of_birth IS NULL`,
      ).catch(() => {});
    ...
    await query(
      `ALTER TABLE payments ADD COLUMN received_amount DECIMAL(14,2) NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD COLUMN payment_reference VARCHAR(80) NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD COLUMN bank_transaction_id VARCHAR(120) NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'completed'`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments MODIFY COLUMN status VARCHAR(20) NOT NULL DEFAULT 'completed'`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD COLUMN expires_at DATETIME NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD COLUMN inventory_deducted_at DATETIME NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD COLUMN webhook_payload LONGTEXT NULL`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD UNIQUE INDEX uq_payments_reference (payment_reference)`,
    ).catch(() => {});
    await query(
      `ALTER TABLE payments ADD UNIQUE INDEX uq_payments_bank_transaction (bank_transaction_id)`,
    ).catch(() => {});
    await query(
...
export const updateOrderStatus = async (
  ...
  if (isClosed) {
    const result = await query<any>(
      "UPDATE orders SET status = ?, closed_at = NOW() WHERE id = ?",
      [status, id],
    );
  ...
  const result = await query<any>("UPDATE orders SET status = ? WHERE id = ?", [
    status,
    id,
  ]);
  return result.affectedRows > 0;
...
export const getTableAreas = async (): Promise<TableArea[]> => {
  try {
    const rows = await query<any[]>(
      "SELECT * FROM table_areas WHERE is_active = 1 ORDER BY id ASC",
    );
    return rows.map((row) => ({
      id: Number(row.id),
      name: row.name,
...
const mapTable = (row: any): any => {
  return {
    ...
    tableNumber:
      row.tableNumber !== undefined ? Number(row.tableNumber) : undefined,
    capacity: Number(row.capacity),
    status: row.status,
  };
};
...
export const getTableById = async (id: string): Promise<any | null> => {
  try {
    ...
    const rows = await query<any[]>(
      "SELECT * FROM tables WHERE id = ? OR name = ?",
      [id, id],
    );
    return rows[0] ? mapTable(rows[0]) : null;
  }
...
export const getTablesByStatus = async (status: string): Promise<Table[]> => {
  const rows = await query<any[]>(
    "SELECT * FROM tables WHERE status = ? ORDER BY tableNumber ASC",
    [status],
  );
...
export const toggleMenuItemAvailability = async (
  ...
  if (!item) return null;
  await query("UPDATE menu_items SET available = ? WHERE id = ?", [
    available ? 1 : 0,
    id,
  ]);
...
export const getInventory = async (): Promise<Inventory[]> => {
  const rows = await query<any[]>(
    "SELECT * FROM inventory_items ORDER BY itemName ASC",
  );
...
export const getIngredients = async (): Promise<any[]> => {
  const rows = await query<any[]>(
    "SELECT id, name, unit, current_stock as stock, min_stock as threshold FROM ingredients WHERE is_deleted = 0 ORDER BY name ASC",
  );
  return rows.map((r) => ({
    id: String(r.id),
    name: r.name,
...
export const updateInventoryItem = async (
  ...
  await query(
    ...
    [
      ...
      updated.unit,
      updated.minQuantity,
      updated.supplier || null,
      updated.lastRestocked || null,
      id,
    ],
  );
  return updated;
...
export const deleteInventoryItem = async (id: string): Promise<boolean> => {
  const result = await query<any>("DELETE FROM inventory_items WHERE id = ?", [
    id,
  ]);
  return result.affectedRows > 0;
};
...
export const updateInventoryQuantity = async (
  ...
  await query(
    "UPDATE inventory_items SET quantity = ?, lastRestocked = ? WHERE id = ?",
    [newQuantity, lastRestocked, id],
  );
...
export const getLowStockItems = async (): Promise<Inventory[]> => {
  const rows = await query<any[]>(
    "SELECT * FROM inventory_items WHERE quantity <= minQuantity ORDER BY itemName ASC",
  );
  return rows.map(normalizeInventory);
};
...
export const getPaymentById = async (id: string): Promise<Payment | null> => {
  if (!dbAvailable)
    return MOCK_PAYMENTS.find((p) => String(p.id) === String(id)) || null;
  const rows = await query<any[]>(
    `
    SELECT p.id,
           COALESCE(i.order_id, p.invoice_id) AS orderId,
...
export const getPaymentsByOrderId = async (
  ...
  if (!dbAvailable)
    return MOCK_PAYMENTS.filter((p) => String(p.orderId) === String(orderId));
  const rows = await query<any[]>(
    `
    SELECT p.id,
           COALESCE(i.order_id, p.invoice_id) AS orderId,
...
const createPendingBankTransferForLockedInvoice = async (
  ...
  if (!createdPayment?.expires_at)
    throw new Error("Không thể tạo phiên thanh toán QR.");
  return {
...
export const createPendingBankTransferPayment = async (
  ...
  return withTransaction(async (connection) => {
    ...
    return createPendingBankTransferForLockedInvoice(
      connection,
      invoice,
      paymentReference,
    );
...
export const createPayment = async (
  ...
  try {
    ...
    const subVal = Number(
      notesData.subtotal !== undefined
        ? notesData.subtotal
        : payment.amount || 0,
    );
    const taxVal = Number(notesData.vat !== undefined ? notesData.vat : 0);
    const discVal =
      Number(notesData.voucher || 0) + Number(notesData.pointsDiscount || 0) ||
      Number(payment.discountAmount || 0);
    const totVal = Number(
      notesData.finalAmount !== undefined
        ? notesData.finalAmount
        : payment.amount || 0,
    );
    ...
    if (invRows && invRows.length > 0) {
      ...
      const resInv = await query<any>(
        ...
        [Number(payment.orderId) || null, subVal, discVal, taxVal, totVal],
      );
    ...
    console.warn(
      "[createPayment] fallback invoice logic error:",
      (errInv as Error).message,
    );
  }
  ...
  if (
    !["cash", "bank_transfer", "card", "momo", "vnpay"].includes(methodEnum)
  ) {
    methodEnum = "cash";
  }
  ...
  const resPay = await query<any>(
    ...
    [
      invoiceId || Number(payment.orderId) || 1,
      methodEnum,
...
export const getPaymentStatistics = async (
  ...
  const result = rows[0] || {};
  const averageAmount =
    result.totalPayments > 0 ? result.totalAmount / result.totalPayments : 0;
  return {
    ...result,
    averageAmount,
    dateRange: { startDate: startDate || null, endDate: endDate || null },
...
export const getResmanagerTableById = async (
  id: number | string,
): Promise<any | null> => {
  let targetId: any = id;
  const num = Number(id);
...
export const updateResmanagerTableStatus = async (
  ...
  try {
    ...
    const mergedPrimaryTableId = await resolveMergedTableRootInTransaction(
      connection,
      selectedTable.id,
    );
    ...
    const updatedTableIds = [
      ...new Set([...clusterRows, ...groupRows].map((table) => table.id)),
    ];
    if (updatedTableIds.length === 0) {
      throw new TableMergeValidationError(
        "Không tìm thấy cụm bàn cần cập nhật.",
      );
    }
    ...
    if (status === TABLE_STATUS.PENDING_PAYMENT) {
      ...
      await connection
        .query(
         ...
         WHERE primary_table_id = ? AND status = ?`,
          [
            GROUP_SEATING_STATUS.RESOLVED,
            primaryTableId,
            GROUP_SEATING_STATUS.ACTIVE,
          ],
        )
        .catch(() => {});
      await connection
...
export const updateResmanagerTable = async (
  ...
  if (table.area_id !== undefined) {
    fields.push("area_id = ?");
    params.push(table.area_id);
  }
  ...
  if (table.capacity !== undefined) {
    ...
    params.push(table.capacity);
  }
  if (table.row_pos !== undefined) {
    fields.push("row_pos = ?");
    params.push(table.row_pos.toUpperCase());
  }
  if (table.col_pos !== undefined) {
    fields.push("col_pos = ?");
    params.push(table.col_pos);
  }
...
export const hasActiveOrdersForTable = async (
  tableId: number,
): Promise<boolean> => {
  const rows = await query<any[]>(
...
export const hasActiveBookingsForTable = async (
  tableId: number,
): Promise<boolean> => {
...
export const getWalkInBookingConflictForTable = async (
  ...
  const querySql = excludeBookingId
  ...
  const rows = await query<WalkInBookingConflictRow[]>(querySql, params);
  return rows[0] ?? null;
};
...
export type BookingAllocationKind =
  (typeof BOOKING_ALLOCATION_KIND)[keyof typeof BOOKING_ALLOCATION_KIND];
...
const toAvailableBookingTable = (
  table: BookingAllocationTable,
): AvailableBookingTable => ({
  id: table.id,
  name: table.name,
...
const addBookingAllocationOption = (
  ...
  if (tables.length === 0 || totalCapacity < partySize || uniqueKeys.has(key))
    return;
...
const getBestSeparateBookingAllocation = (
  ...
  const largestTableCapacity = Math.max(
    ...tables.map((table) => table.capacity),
  );
  const maximumRelevantCapacity = partySize + largestTableCapacity - 1;
  const combinationsByCapacity = new Map<number, BookingAllocationTable[]>([
    [0, []],
  ]);

  for (const table of [...tables].sort(
    (left, right) =>
      right.capacity - left.capacity || left.name.localeCompare(right.name),
  )) {
    ...
    for (const [capacity, combination] of currentCombinations) {
      ...
      if (
        !existingCombination ||
        nextCombination.length < existingCombination.length
      ) {
  ...
  const bestMatch = [...combinationsByCapacity.entries()]
    ...
    .sort(
      ([leftCapacity, leftTables], [rightCapacity, rightTables]) =>
...
export const getAvailableBookingTableOptions = async (
  ...
  for (const table of tables.filter((item) => item.capacity >= partySize)) {
    addBookingAllocationOption(
      options,
      uniqueKeys,
      [table],
      partySize,
      BOOKING_ALLOCATION_KIND.SINGLE,
    );
  }
  ...
  for (const rowTables of tablesByRow.values()) {
    const sortedTables = [...rowTables].sort(
      (left, right) => (left.col_pos ?? 0) - (right.col_pos ?? 0),
    );
    for (
      let startIndex = 0;
      startIndex < sortedTables.length;
      startIndex += 1
    ) {
      ...
      for (let index = startIndex; index < sortedTables.length; index += 1) {
        ...
        if (previous && (current.col_pos ?? 0) - (previous.col_pos ?? 0) > 1)
          break;
        ...
        addBookingAllocationOption(
          ...
          candidate,
          partySize,
          BOOKING_ALLOCATION_KIND.ADJACENT,
        );
  ...
  addBookingAllocationOption(
    ...
    separateCandidate,
    partySize,
    BOOKING_ALLOCATION_KIND.SEPARATE,
  );
  ...
  return options.sort((left, right) => {
    ...
    return (
      kindRank[left.allocationKind] - kindRank[right.allocationKind] ||
...
export const getBookings = async (status?: string): Promise<any[]> => {
  ...
  const assignmentMap = new Map<
    number,
    { table_id: number; table_name: string; allocated_capacity: number }[]
  >();
  ...
  for (const assignment of assignments) {
    const currentAssignments =
      assignmentMap.get(Number(assignment.booking_id)) ?? [];
  ...
  for (const row of rows) {
    ...
    if (tableAssignments.length > 0) {
      ...
      row.table_names = tableAssignments
        .map((assignment) => assignment.table_name)
        .join(", ");
...
export const getBookingSchedule = async (
  ...
  if (filters.tableId !== undefined) {
    conditions.push(
      "(bta.table_id = ? OR (bta.booking_id IS NULL AND b.table_id = ?))",
    );
  ...
  const orderByClause =
    filters.mode === BOOKING_SCHEDULE_MODE.HISTORY
...
export const getBookingById = async (id: number): Promise<any | null> => {
  ...
  booking.table_ids = tableAssignments.map((assignment) =>
    Number(assignment.table_id),
  );
  booking.table_names = tableAssignments
    .map((assignment) => assignment.table_name)
    .join(", ");
...
export const createBooking = async (data: any): Promise<any> => {
  ...
  if (requestedTableIds.length === 0) {
    ...
    if (
      !Number.isInteger(requestedPrimaryTableId) ||
      !requestedTableIds.includes(requestedPrimaryTableId)
    ) {
      throw new Error("Cụm bàn đặt trước không hợp lệ.");
    }
    const requestedOptionKey = requestedTableIds
      .sort((left, right) => left - right)
      .join(",");
    const selectedTableIsAvailable = availableOptions.some(
      (option) =>
        option.primaryTable.id === requestedPrimaryTableId &&
        getBookingAllocationKey(option.tables) === requestedOptionKey,
    );
    if (!selectedTableIsAvailable) {
      throw new Error(
        "Bàn không còn trống hoặc không đủ sức chứa trong khung giờ đã chọn.",
      );
    }
  }
  ...
  try {
    ...
    await connection.query(
      `SELECT id FROM tables WHERE id IN (${tablePlaceholders}) FOR UPDATE`,
      requestedTableIds,
    );
    ...
    // Kiểm tra trùng lịch đặt bàn sau khi khóa bàn để chống overbooking giữa các request đồng thời.
    {
      ...
      if (overlaps.length > 0) {
        throw new Error(
          "Khung giờ đặt bàn này đã bị trùng với lịch đặt khác trên cùng bàn!",
        );
      }
    }
    ...
    if (data.customer_id) {
      const [custRows] = await connection.query<any[]>(
        "SELECT id FROM customers WHERE id = ? AND is_deleted = 0 LIMIT 1",
        [data.customer_id],
      );
    ...
    if (!validCustomerId && data.guest_phone) {
      const [custByPhone] = await connection.query<any[]>(
        "SELECT id FROM customers WHERE phone = ? AND is_deleted = 0 LIMIT 1",
        [data.guest_phone],
      );
    ...
    if (data.guest_phone || validCustomerId) {
      ...
      `,
        validCustomerId
          ? [data.guest_phone, validCustomerId, data.end_time, data.start_time]
          : [data.guest_phone, data.end_time, data.start_time],
      );

      if (customerOverlaps.length > 0) {
        throw new Error(
          `Số điện thoại ${data.guest_phone} đã có một lịch đặt bàn khác trong khoảng thời gian này. Vui lòng chọn khung giờ khác!`,
        );
      }
    }
    ...
    if (data.promotion_id) {
      const [promoRows] = await connection.query<any[]>(
    ...
    if (preOrderedItems.length > 0) {
      ...
      depositAmount = preOrderTotal * 0.2;
    ...
    const assignmentPlaceholders = assignedTables
      .map(() => "(?, ?, ?, ?)")
      .join(",");
    ...
    if (preOrderedItems.length > 0) {
      ...
      `,
        [
          ...
          1,
          data.guest_note
            ? `[Booking ${code}] ${data.guest_note}`
            : `[Booking ${code}] Đơn đặt món trước`,
          data.guest_name,
          data.guest_phone,
          data.party_size,
        ],
...
export const transferBookingItemsToOrder = async (
  tableId: number,
  orderId: string,
): Promise<void> => {
  ...
  if (items.length > 0) {
    ...
    console.log(
      `✅ Transferred ${items.length} items from booking ${bookingId} to order ${orderId}`,
    );
...
export const checkInScheduledBooking = async (
  ...
  try {
    ...
    const checkInState = String(
      checkInWindowRows[0]?.check_in_state ?? "expired",
    );
    if (checkInState === "before") {
      throw new Error(
        `Chỉ có thể mở bàn trước giờ đặt ${BOOKING_CHECK_IN_EARLY_MINUTES} phút.`,
      );
    }
    if (checkInState === "expired") {
      throw new Error(
        "Đã quá thời gian nhận khách của lịch đặt này. Vui lòng tạo booking mới hoặc liên hệ quản lý.",
      );
    }
    ...
    const bookingTableIds = [
      ...new Set(bookingTableRows.map((row) => Number(row.id))),
    ];
    if (bookingTableIds.length === 0) {
      throw new Error("Lịch đặt chưa được gán bàn.");
    }
    const tablePlaceholders = bookingTableIds.map(() => "?").join(", ");
    const [physicalTableRows] = await connection.query<
      BookingCheckInPhysicalTableRow[]
    >(
    ...
    const unavailableTables = physicalTableRows.filter(
      (table) =>
        table.status !== TABLE_STATUS.EMPTY &&
        table.status !== TABLE_STATUS.RESERVED,
    );
    if (unavailableTables.length > 0) {
      const tableNames = unavailableTables
    ...
    if (activeOrderRows.length > 0) {
      throw new Error(
        "Bàn vẫn đang phục vụ khách trước. Không thể mở lịch đặt kế tiếp cho đến khi bàn được thanh toán và dọn xong.",
      );
    }

    const primaryTableId = booking.table_id || bookingTableIds[0];
    ...
    if (preOrder) {
      ...
      const [preOrderedItemRows] = await connection.query<
        mysql.RowDataPacket[]
      >(
...
export const updateBookingStatus = async (
  ...
  if (status === "cancelled") {
    ...
    const tableIds = [
      ...new Set(
        assignedTableRows.map((r: any) => Number(r.table_id)).filter(Boolean),
      ),
    ];
...
export const ensureEarlyPaymentColumns = async (): Promise<void> => {
  try {
    ...
    if (!earlyPaymentColumn || earlyPaymentColumn.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN is_early_payment TINYINT(1) NOT NULL DEFAULT 0`,
      ).catch(() => {});
      await query(
        `ALTER TABLE orders ADD COLUMN is_early_paid TINYINT(1) NOT NULL DEFAULT 0`,
      ).catch(() => {});
      console.log(
        "Migration: added orders.is_early_payment and orders.is_early_paid",
      );
...
export const ensureRefundColumns = async (): Promise<void> => {
  try {
    ...
    if (!colOrderItems || colOrderItems.length === 0) {
      await query(
        `ALTER TABLE order_items ADD COLUMN is_refunded TINYINT(1) NOT NULL DEFAULT 0`,
      ).catch(() => {});
      await query(
        `ALTER TABLE order_items ADD COLUMN refunded_at DATETIME NULL`,
      ).catch(() => {});
      await query(
        `ALTER TABLE order_items ADD COLUMN refund_reason VARCHAR(255) NULL`,
      ).catch(() => {});
      await query(
        `ALTER TABLE order_items ADD COLUMN refund_amount DECIMAL(12,2) NOT NULL DEFAULT 0`,
      ).catch(() => {});
    ...
    if (!colOrders || colOrders.length === 0) {
      await query(
        `ALTER TABLE orders ADD COLUMN refunded_total DECIMAL(12,2) NOT NULL DEFAULT 0`,
      ).catch(() => {});
      await query(
        `ALTER TABLE orders ADD COLUMN has_refund TINYINT(1) NOT NULL DEFAULT 0`,
      ).catch(() => {});
...
export const ensureResmanagerTablesSchema = async (): Promise<void> => {
  try {
    ...
    if (tmCols && tmCols.length > 0) {
      ...
      if (!tmSet.has("status")) {
        await query(
          `ALTER TABLE table_merges ADD COLUMN status ENUM('active','resolved') NOT NULL DEFAULT 'active'`,
        ).catch(() => {});
      }
      if (!tmSet.has("primary_order_id")) {
        await query(
          `ALTER TABLE table_merges ADD COLUMN primary_order_id INT NULL`,
        ).catch(() => {});
      }
      if (!tmSet.has("merged_order_id")) {
        await query(
          `ALTER TABLE table_merges ADD COLUMN merged_order_id INT NULL`,
        ).catch(() => {});
      }
      if (!tmSet.has("merged_by")) {
        await query(
          `ALTER TABLE table_merges ADD COLUMN merged_by INT NULL`,
        ).catch(() => {});
      }
      if (!tmSet.has("resolved_at")) {
        await query(
          `ALTER TABLE table_merges ADD COLUMN resolved_at DATETIME NULL`,
        ).catch(() => {});
      }
    }
    ...
    if (bCols && bCols.length > 0) {
      const bSet = new Set(
        bCols.map((c) => String(c.COLUMN_NAME).toLowerCase()),
      );
    ...
    if (tsCols && tsCols.length > 0) {
      ...
      if (!tsSet.has("status")) {
        await query(
          `ALTER TABLE table_splits ADD COLUMN status ENUM('active', 'paid', 'cancelled') NOT NULL DEFAULT 'active'`,
        ).catch(() => {});
      }
      if (!tsSet.has("split_session_id")) {
        await query(
          `ALTER TABLE table_splits ADD COLUMN split_session_id INT NULL`,
        ).catch(() => {});
      }
      if (!tsSet.has("guest_count")) {
        await query(
          `ALTER TABLE table_splits ADD COLUMN guest_count INT NOT NULL DEFAULT 1`,
        ).catch(() => {});
      }
      if (!tsSet.has("closed_at")) {
        await query(
          `ALTER TABLE table_splits ADD COLUMN closed_at DATETIME NULL`,
        ).catch(() => {});
      }
    }
  } catch (err) {
...
export const getResmanagerTablesWithExtra = async (
  areaId?: number,
): Promise<any[]> => {
  ...
  let sql = `
    ...
    LEFT JOIN bookings b ON b.id = COALESCE(
      o.booking_id,
      CASE WHEN t.status = 'reserved' THEN (
        SELECT id FROM bookings
        WHERE table_id = t.id AND status IN ('pending', 'confirmed', 'arrived')
        ORDER BY start_time ASC, id ASC
        LIMIT 1
      ) END,
      CASE WHEN t.status = 'reserved' THEN (
        SELECT booking_id FROM booking_table_assignments
        WHERE table_id = t.id
  ...
  for (const r of rows) {
    ...
    const clusterCapacity = clusterTables.reduce(
    ...
    if (r.active_order_id && r.active_order_type === "pre_order") {
      ...
      r.booking_id &&
      (!r.active_order_id || r.active_order_type !== "pre_order")
    ) {
    ...
    results.push({
      ...
      group_seating_code:
        groupOwner[0]?.group_code ?? groupChildren[0]?.group_code ?? null,
      group_seating_capacity:
        groupChildren.length > 0 || groupOwner.length > 0
          ? groupCapacity
          : null,
...
export const getEmptyTablesForBooking = async (
  startTime?: string,
  endTime?: string,
): Promise<any[]> => {
...
const formatTransferBookingTime = (startTime: Date | string): string =>
...
export const transferResmanagerOrder = async (
  ...
  if (
    !Number.isInteger(sourceTableId) ||
    sourceTableId <= 0 ||
    !Number.isInteger(targetTableId) ||
    targetTableId <= 0
  ) {
    throw new TableTransferValidationError(
      "ID bàn nguồn và bàn đích phải hợp lệ.",
    );
  }
  if (sourceTableId === targetTableId) {
    throw new TableTransferValidationError(
      "Bàn nguồn và bàn đích phải khác nhau.",
    );
  }
  ...
  try {
    ...
    if (tableRows.length !== 2) {
      throw new TableTransferValidationError(
        "Không tìm thấy bàn nguồn hoặc bàn đích.",
      );
    }
    ...
    if (!sourceTable || !targetTable) {
      throw new TableTransferValidationError(
        "Không thể xác định bàn nguồn hoặc bàn đích.",
      );
    }
    if (targetTable.status !== TABLE_STATUS.EMPTY) {
      throw new TableTransferValidationError(
        `Bàn ${targetTable.name} không trống, không thể chuyển khách vào.`,
      );
    }
    ...
    if (
      sourceTable.merged_into_table_id !== null ||
      targetTable.merged_into_table_id !== null ||
      mergeRows.length > 0 ||
      groupRows.length > 0
    ) {
      throw new TableTransferValidationError(
        "Không thể chuyển bàn đang thuộc cụm gộp hoặc đoàn. Hãy tách/hoàn tất cụm trước.",
      );
    }
    ...
    if (sourceOrders.length === 0) {
      throw new TableTransferValidationError(
        `Bàn ${sourceTable.name} không có order đang phục vụ để chuyển.`,
      );
    }
    if (sourceOrders.length > 1) {
      throw new TableTransferValidationError(
        `Bàn ${sourceTable.name} có nhiều order đang hoạt động, cần xử lý dữ liệu trước khi chuyển.`,
      );
    }
    ...
    if (targetOrders.length > 0) {
      throw new TableTransferValidationError(
        `Bàn ${targetTable.name} đang có order hoạt động, không thể chuyển khách vào.`,
      );
    }

    const bookingWindowEnd = new Date(
      Date.now() + MERGE_BOOKING_LOOKAHEAD_MINUTES * 60 * 1000,
    );
    const [upcomingBookings] = await connection.query<
      TableTransferBookingRow[]
    >(
...
export const mergeResmanagerTables = async (
  ...
  let primaryOrders = await query<any[]>(
    "SELECT id FROM orders WHERE table_id = ? AND status IN ('open', 'serving', 'pending_payment') ORDER BY id DESC LIMIT 1",
    [primaryTableId],
  );
  ...
  for (const mergedId of mergedTableIds) {
    ...
    await query("UPDATE tables SET status = 'serving' WHERE id = ?", [
      mergedId,
    ]);
    ...
    const sourceGuestName =
      mOrderInfo?.guest_name || mBookingInfo?.guest_name || null;
    const sourceGuestPhone =
      mOrderInfo?.guest_phone || mBookingInfo?.guest_phone || null;
    const sourceGuestCount = Number(
      mOrderInfo?.guest_count || mBookingInfo?.guest_count || 0,
    );
    const sourceCustomerId =
      mOrderInfo?.customer_id || mBookingInfo?.customer_id || null;
    ...
    for (const mOrder of mergedOrders) {
      if (!primaryOrderId) {
        ...
        await query("UPDATE orders SET table_id = ? WHERE id = ?", [
          primaryTableId,
          mOrder.id,
        ]);
        ...
        await query("UPDATE order_items SET order_id = ? WHERE order_id = ?", [
          primaryOrderId,
          mOrder.id,
        ]);
        await query(
          "UPDATE orders SET status = 'cancelled', note = CONCAT(COALESCE(note, ''), ' [Gộp vào bàn chính #${primaryTableId}]') WHERE id = ?",
          [mOrder.id],
        );
      }
    }
    ...
    if (
      !primaryOrderId &&
      (sourceGuestName || sourceGuestPhone || sourceGuestCount > 0)
    ) {
      const insertRes = await query<any>(
        ...
        [
          primaryTableId,
          sourceGuestName || null,
          sourceGuestPhone || null,
          sourceGuestCount || 0,
    ...
    if (
      ...
      const [currentPrimary] = await query<any[]>(
        "SELECT * FROM orders WHERE id = ?",
        [primaryOrderId],
      );
      if (currentPrimary) {
        const isDefaultName =
          !currentPrimary.guest_name ||
          currentPrimary.guest_name === "Khách tại bàn" ||
          currentPrimary.guest_name === "Khách lẻ";
        const updatedGuestName = isDefaultName
          ? sourceGuestName
          : currentPrimary.guest_name;
        const updatedGuestPhone =
          currentPrimary.guest_phone || sourceGuestPhone;
        const updatedCustomerId =
          currentPrimary.customer_id || sourceCustomerId;
        const updatedGuestCount =
          Number(currentPrimary.guest_count || 0) + sourceGuestCount ||
          sourceGuestCount ||
          Number(currentPrimary.guest_count || 0);
        const updatedNote = [currentPrimary.note, sourceNote]
          .filter(Boolean)
          .join(" | ");
        ...
        await query(
          ...
          [
            updatedGuestName || null,
            updatedGuestPhone || null,
...
export const unmergeResmanagerTable = async (
  primaryTableId: number,
): Promise<boolean> => {
  const mergedTables = await query(
  ...
  for (const m of mergedTables) {
    ...
    if (activeOrders.length === 0) {
      await query("UPDATE tables SET status = 'empty' WHERE id = ?", [
        m.merged_table_id,
      ]);
    }
  }
  await query("DELETE FROM table_merges WHERE primary_table_id = ?", [
    primaryTableId,
  ]);
  return true;
};
...
const areTablesPhysicallyAdjacent = (
  firstTable: MergeTableRow,
  secondTable: MergeTableRow,
): boolean => {
  ...
  const rowDistance = Math.abs(
    firstTable.row_pos.charCodeAt(0) - secondTable.row_pos.charCodeAt(0),
  );
  const columnDistance = Math.abs(firstTable.col_pos - secondTable.col_pos);

  return (
    (sameRow && columnDistance === ADJACENT_TABLE_GRID_DISTANCE) ||
...
const assertMergeTablesAreContiguous = (
  ...
  while (pendingTables.length > 0) {
    const nextTableIndex = pendingTables.findIndex((candidateTable) =>
      connectedTables.some((connectedTable) =>
        areTablesPhysicallyAdjacent(connectedTable, candidateTable),
      ),
    );
    if (nextTableIndex < 0) {
      const disconnectedNames = pendingTables
        .map((table) => table.name)
        .join(", ");
      throw new TableMergeValidationError(
...
const resolveMergedTableRootInTransaction = async (
  ...
  while (true) {
    if (visitedTableIds.has(currentTableId)) {
      throw new TableMergeValidationError(
        "Phát hiện vòng lặp dữ liệu gộp bàn. Vui lòng liên hệ quản lý.",
      );
    }
...
export const resolveResmanagerPrimaryTableId = async (
  tableId: number,
): Promise<number> => {
  let currentTableId = tableId;
  const visitedTableIds = new Set<number>();
...
export const getResmanagerActiveOrderForTable = async (
  tableId: number,
...
export const arrangeGroupSeatingTransactionally = async (
  ...
  if (
    ...
    assignedTableIds.some((id) => !Number.isInteger(id) || id <= 0)
  ) {
    throw new TableMergeValidationError(
      "Danh sách bàn xếp cho đoàn không hợp lệ.",
    );
  }
  ...
  try {
    await connection.beginTransaction();
    const primaryTableId = await resolveMergedTableRootInTransaction(
      connection,
      requestedPrimaryTableId,
    );
    if (
      assignedTableIds.includes(primaryTableId) ||
      assignedTableIds.includes(requestedPrimaryTableId)
    ) {
      throw new TableMergeValidationError(
        "Không thể xếp bàn chính vào chính đoàn của nó.",
      );
    }

    const primaryClusterIdsRows = await connection.query<
      GroupSeatingTableRow[]
    >(
      `SELECT id, capacity
       FROM tables
       WHERE (id = ? OR merged_into_table_id = ?) AND is_deleted = 0
    ...
    const allowedPrimaryStatuses = new Set<string>([
      TABLE_STATUS.SERVING,
      TABLE_STATUS.RESERVED,
    ]);
    if (!allowedPrimaryStatuses.has(primaryTable.status)) {
      throw new TableMergeValidationError(
        `Bàn ${primaryTable.name} phải đang phục vụ hoặc đã đặt trước để xếp bàn đoàn.`,
      );
    }
    ...
    if (assignedTables.length !== assignedTableIds.length) {
      throw new TableMergeValidationError(
        "Có bàn xếp cho đoàn không tồn tại hoặc đã bị xóa.",
      );
    }
    ...
    if (physicalMergeRows.length > 0) {
      throw new TableMergeValidationError(
        "Bàn đã thuộc một cụm gộp vật lý không thể được xếp riêng cho đoàn.",
      );
    }

    const involvedPlaceholders = [...primaryClusterIds, ...assignedTableIds]
      .map(() => "?")
      .join(", ");
    ...
    if (existingGroupRows.length > 0) {
      throw new TableMergeValidationError(
        "Một trong các bàn đã thuộc đoàn khác. Hãy hoàn tất hoặc tách đoàn hiện tại trước.",
      );
    }
    ...
    for (const assignedTable of assignedTables) {
      if (assignedTable.status !== TABLE_STATUS.EMPTY) {
        throw new TableMergeValidationError(
          `Bàn ${assignedTable.name} phải ở trạng thái trống để xếp cho đoàn.`,
        );
      }
    }

    const activeOrderStatusPlaceholders = ACTIVE_ORDER_STATUSES.map(
      () => "?",
    ).join(", ");
    const [guestCountRows] = await connection.query<
      GroupSeatingGuestCountRow[]
    >(
      `SELECT guest_count
       FROM orders
    ...
    if (guestCount <= 0) {
      const [bookingGuestRows] = await connection.query<
        GroupSeatingGuestCountRow[]
      >(
    ...
    if (!Number.isInteger(guestCount) || guestCount <= 0) {
      throw new TableMergeValidationError(
        "Chưa có số lượng khách hợp lệ trên bàn chính nên chưa thể xếp bàn đoàn.",
      );
    }

    const totalCapacity = [...primaryClusterRows, ...assignedTables].reduce(
      (total, table) => total + Number(table.capacity),
      0,
    );
    if (guestCount > totalCapacity) {
      throw new TableMergeValidationError(
        `Tổng sức chứa ${totalCapacity} chỗ vẫn không đủ cho đoàn ${guestCount} khách.`,
      );
    }
...
export const mergeResmanagerTablesTransactionally = async (
  ...
  const uniqueMergedTableIds = [
    ...new Set(requestedMergedTableIds.map(Number)),
  ];
  if (
    uniqueMergedTableIds.length === 0 ||
    uniqueMergedTableIds.some((id) => !Number.isInteger(id) || id <= 0)
  ) {
  ...
  try {
    ...
    if (
      uniqueMergedTableIds.includes(primaryTableId) ||
      uniqueMergedTableIds.includes(requestedPrimaryTableId)
    ) {
      throw new TableMergeValidationError(
        "Bàn chính không thể gộp vào chính nó.",
      );
    }
    ...
    if (tableRows.length !== tableIds.length) {
      throw new TableMergeValidationError(
        "Có bàn không tồn tại hoặc đã bị xóa.",
      );
    }
    ...
    if (!allowedStatuses.has(primaryTable.status)) {
      throw new TableMergeValidationError(
        `Bàn ${primaryTable.name} đang ở trạng thái không thể gộp.`,
      );
    }
    ...
    if (splitSessions.length > 0) {
      throw new TableMergeValidationError(
        "Không thể gộp bàn đang có phiên tách bàn (Sub-Orders) hoạt động.",
      );
    }
    ...
    for (const mergedTable of mergedTables) {
      if (mergedTable.area_id !== primaryTable.area_id) {
        throw new TableMergeValidationError(
          "Chỉ có thể gộp các bàn trong cùng khu vực.",
        );
      }
      if (!allowedStatuses.has(mergedTable.status)) {
        throw new TableMergeValidationError(
          `Bàn ${mergedTable.name} đang ở trạng thái không thể gộp.`,
        );
      }
      if (mergedTable.merged_into_table_id !== null) {
        throw new TableMergeValidationError(
          `Bàn ${mergedTable.name} đã thuộc một cụm bàn khác.`,
        );
      }
    }

    assertMergeTablesAreContiguous(
      primaryTable,
      existingMergedTables,
      mergedTables,
    );
    ...
    if (blockingBooking) {
      const bookingTime = new Date(
        blockingBooking.start_time,
      ).toLocaleTimeString("vi-VN", {
        hour: "2-digit",
        minute: "2-digit",
      });
      throw new TableMergeValidationError(
        `Bàn ${blockingBooking.table_name} có lịch khách đặt lúc ${bookingTime}.`,
      );
    }
    ...
    if (
      primaryOrders.some(
        (order) => order.status === ORDER_STATUS.PENDING_PAYMENT,
      )
    ) {
      throw new TableMergeValidationError(
        "Không thể gộp bàn chính đang chờ thanh toán.",
      );
    }

    const mergedClusterCapacity = [
      primaryTable,
      ...existingMergedTables,
      ...mergedTables,
    ].reduce((total, table) => total + Number(table.capacity), 0);
    const guestCountScopeTableIds = [primaryTableId, ...uniqueMergedTableIds];
    const guestCountScopePlaceholders = guestCountScopeTableIds
      .map(() => "?")
      .join(", ");
    ...
    for (const mergedTable of mergedTables) {
      ...
      if (
        ...
        throw new TableMergeValidationError(
          `Bàn ${mergedTable.name} đang chờ thanh toán nên không thể gộp.`,
        );
      }
      ...
      for (const sourceOrder of sourceOrders) {
        ...
        await connection.query(
          ...
          [
            ORDER_STATUS.MERGED,
            primaryOrderId,
            `Đã gộp vào order #${primaryOrderId}`,
            sourceOrder.id,
          ],
        );
    ...
    return {
      primaryTableId,
      mergedTableIds: uniqueMergedTableIds,
      primaryOrderId,
    };
  } catch (error) {
    await connection.rollback();
...
export const releaseMergedTableClusterAfterPayment = async (
  tableId: number,
): Promise<number[]> => {
  const connection = await ensurePool().getConnection();
  try {
    ...
    const clusterTableIds = [
      ...new Set([...clusterRows, ...groupRows].map((table) => table.id)),
    ];
    if (clusterTableIds.length === 0) {
      throw new TableMergeValidationError(
        "Không tìm thấy cụm bàn cần giải phóng.",
      );
    }
...
export const unmergeResmanagerTablesTransactionally = async (
  tableId: number,
): Promise<number[]> => {
  const connection = await ensurePool().getConnection();
  try {
    ...
    if (activeOrders.length > 0) {
      throw new TableMergeValidationError(
        "Không thể bỏ gộp sau khi đã phát sinh order. Hãy tách món theo nghiệp vụ.",
      );
    }
    ...
    if (mergedTableIds.length === 0) {
      throw new TableMergeValidationError(
        "Bàn này không có cụm bàn phụ để bỏ gộp.",
      );
    }
...
export const checkMenuItemAvailability = async (
  menuItemId: number,
): Promise<{
  available: boolean;
  reason?: string;
  ...
  if (!menuCheck[0].is_active) {
    return {
      available: false,
      reason: `Món '${menuItemName}' đang tạm ngưng phục vụ!`,
    };
  }
...
export const getResmanagerMenuItems = async (
  categoryId?: number,
): Promise<any[]> => {
  let sql = `
...
export const getResmanagerOrdersByTable = async (
  tableId: number,
): Promise<any[]> => {
...
export const getAllResmanagerOrders = async (
  status?: string,
): Promise<any[]> => {
  let sql = `
    ...
    if (isPaidOrder) {
      ...
      if (payRows && payRows.length > 0) {
        ...
        try {
          if (
            pRow.note &&
            typeof pRow.note === "string" &&
            pRow.note.startsWith("{")
          ) {
            notesData = JSON.parse(pRow.note);
          }
        } catch {}
        order.tax = Number(
          notesData.vat !== undefined ? notesData.vat : pRow.tax || 0,
        );
        order.voucherDiscount = Number(
          notesData.voucher !== undefined
            ? notesData.voucher
            : pRow.discount || 0,
        );
        ...
        order.totalAmount = Number(
          notesData.finalAmount !== undefined
            ? notesData.finalAmount
            : pRow.total || pRow.amount || subtotal,
        );
      } else {
        const invRows = await query<any[]>(
      ...
      order.totalAmount = Math.max(
        0,
        subtotal + order.tax - order.depositAmount - order.discount,
      );
    }
  }
  return orders;
...
export const deductInventoryForItem = async (
  ...
  try {
    ...
    if (item.is_reused) {
      console.log(
        `[Inventory] Bỏ qua trừ kho cho món tái sử dụng (ID: ${orderItemId})`,
      );
      return;
    }
    ...
    for (const recipe of recipeItems) {
      ...
      for (const batch of batches) {
        ...
        const defaultNote =
          reason === "waste"
            ? "Hao hụt do hủy món (nấu món)"
            : "Trừ kho tự động theo FEFO (nấu món)";
        await query(
      ...
      if (remainingToDeduct > 0) {
        const defaultGenericNote =
          reason === "waste"
            ? "Hao hụt tự động (âm kho/không rõ lô)"
            : "Trừ kho tự động (âm kho/không rõ lô)";
        await query(
    ...
    console.error(
      `❌ Error deducting inventory for item ${orderItemId}:`,
      error.message,
    );
  }
};

export const refundInventoryForItem = async (
  orderItemId: string | number,
): Promise<void> => {
  try {
    if (!dbAvailable) return;
    ...
    console.error(
      `❌ Error refunding inventory for item ${orderItemId}:`,
      error.message,
    );
  }
};
 ...
 */
export const processReuseForOrderItems = async (
  orderItemIds: number[],
): Promise<void> => {
  try {
    ...
    for (const item of items) {
      ...
      await createNewDishNotification(
        item.order_id,
        item.menu_item_id,
        item.quantity,
      );
...
export const reuseCancelledKdsItem = async (
  ...
  if (cancelledItems.length === 0 || targetItems.length === 0) {
    throw new Error(
      "Không tìm thấy món ăn nguồn (hủy) hoặc món ăn đích (chờ).",
    );
  }
  ...
  if (!cItem.is_cooked_cancelled) {
    throw new Error(
      "Món ăn nguồn chưa được đánh dấu là đã nấu/đang nấu bị hủy.",
    );
  }
  ...
  if (CQ <= TQ) {
    ...
    if (CQ === TQ) {
      ...
      await query(`UPDATE order_items SET quantity = ? WHERE id = ?`, [
        remainQty,
        tItem.id,
      ]);
...
export const getResmanagerPayments = async (): Promise<any[]> => {
  ...
  return rows.map((row) => {
    ...
    remainingRefundByOrder.set(
      orderKey,
      Math.max(0, remainingRefund - allocatedRefund),
    );
...
export const getResmanagerOrderItems = async (
  orderId: number,
): Promise<any[]> => {
  return query(
    `
...
export const createResmanagerOrder = async (data: any): Promise<any> => {
  if (data.table_id) {
    data.table_id = await resolveResmanagerPrimaryTableId(
  ...
  if (!validCustomerId && data.guest_phone) {
    const custByPhone = await query<any[]>(
      "SELECT id FROM customers WHERE phone = ? AND is_deleted = 0 LIMIT 1",
      [data.guest_phone],
    );
    if (custByPhone.length > 0) {
  ...
  if (data.table_id) {
    ...
    if (existingPreOrders.length > 0) {
      ...
      if (bookingIdNum) {
        await query("UPDATE bookings SET status = 'arrived' WHERE id = ?", [
          bookingIdNum,
        ]).catch(() => {});
      }
      return {
...
export const completeActiveBookingForTable = async (
  tableId: number,
): Promise<boolean> => {
  const result = await query(
    `
...
export const addResmanagerOrderItem = async (data: any): Promise<any> => {
  if (data.menu_item_id) {
    const availCheck = await checkMenuItemAvailability(
      Number(data.menu_item_id),
    );
    if (!availCheck.available) {
      throw new Error(
        availCheck.reason ||
          "Món ăn đã hết hàng hoặc nguyên liệu trong kho đã hết hạn!",
      );
    }
  }
  ...
  if (!data.bypass_status_check && data.order_id) {
    ...
    if (orderCheck && orderCheck.length > 0) {
      ...
      if (
        ...
        throw new Error(
          "Bàn đang yêu cầu thanh toán (Chờ thanh toán). Hệ thống đã khóa gọi thêm món để tránh sai lệch hóa đơn!",
        );
      }
    }
  }
  ...
  if (existingRows.length > 0) {
    ...
    if (!itemCreator && data.order_id) {
      try {
        const parentOrder = await query<any[]>(
          `SELECT created_by FROM orders WHERE id = ? LIMIT 1`,
          [data.order_id],
        );
...
export const voidResmanagerOrderItem = async (
  itemId: number,
  reason: string,
): Promise<boolean> => {
  ...
  if (currentStatus === "cooking") {
    await deductInventoryForItem(
      itemId,
      "waste",
      "Hao hụt do hủy món đang nấu",
    );
  }
...
export const sendResmanagerOrderItemsToKitchen = async (
  orderItemIds: number[],
): Promise<boolean> => {
  ...
  for (const item of items) {
    ...
    if (!availCheck.available) {
      throw new Error(
        availCheck.reason ||
          `Không thể gửi món '${item.item_name}' xuống bếp do hết hàng hoặc nguyên liệu đã hết hạn!`,
      );
    }
  }
...
export const holdResmanagerOrderItems = async (
  itemIds: number[],
  held: boolean,
): Promise<boolean> => {
...
export const getEventPackages = async (): Promise<any[]> => {
  const pkgs = await query<any[]>(
    "SELECT * FROM event_packages WHERE is_active = 1",
  );
  for (const p of pkgs) {
    p.items = await query(
      `
...
export const updateEventPackage = async (
  ...
  if (Object.keys(pkgData).length > 0) {
    ...
    values.push(id);
    await query(
      `UPDATE event_packages SET ${fields.join(", ")} WHERE id = ?`,
      values,
    );
  }
...
export const getEventById = async (
  ...
  event.services = await query(
    "SELECT * FROM event_services WHERE event_id = ?",
    [id],
  );
...
export const createResmanagerUser = async (data: any): Promise<any> => {
  const hourlyRate =
    data.hourly_rate !== undefined ? Number(data.hourly_rate) : 25000;
  const result = await query(
    `
  ...
  const insertId = result.insertId;
  const empCode =
    data.employee_code || `NV${String(insertId).padStart(3, "0")}`;
  await query("UPDATE users SET employee_code = ? WHERE id = ?", [
    empCode,
    insertId,
  ]);
...
export const updateResmanagerUser = async (
  id: number | string,
  data: any,
): Promise<boolean> => {
  const fields: string[] = [];
...
export const notifyWaitlistGuest = async (id: number): Promise<boolean> => {
  const result = await query(
    "UPDATE waitlist SET notified_at = NOW() WHERE id = ?",
    [id],
  );
  return result.affectedRows > 0;
};
...
export const findCustomerByEmail = async (
  ...
  if (!dbAvailable)
    return (
      MOCK_CUSTOMERS.find((c) => c.email === email && !c.is_deleted) || null
    );
  const rows = await query(
    "SELECT * FROM customers WHERE email = ? AND is_deleted = 0",
    [email],
  );
  return rows[0] || null;
};
...
export const findCustomerById = async (
  ...
  if (!dbAvailable)
    return MOCK_CUSTOMERS.find((c) => c.id === id && !c.is_deleted) || null;
  const rows = await query(
...
export const redeemCustomerVoucher = async (
  customerId: number,
  voucherId: number,
): Promise<any> => {
  const voucherRows = await query<any[]>(
  ...
  if (
    voucher.expired_at &&
    new Date(voucher.expired_at).getTime() < Date.now()
  ) {
  ...
  if (currentPoints < pointsReq) {
    throw new Error(
      `Bạn cần có tối thiểu ${pointsReq} điểm để đổi voucher này! (Số điểm hiện tại: ${currentPoints} PTS)`,
    );
  }
  ...
  `,
    [
      customerId,
      pointsReq,
      `Đổi mã ưu đãi ${voucher.code} (-${pointsReq} PTS)`,
    ],
  );

  await query("UPDATE vouchers SET used_count = used_count + 1 WHERE id = ?", [
    voucherId,
  ]);
...
export const getCustomerVouchers = async (): Promise<any[]> => {
  return query(
    "SELECT * FROM vouchers WHERE is_active = 1 AND (expired_at IS NULL OR expired_at > NOW())",
  );
};

/** Returns the tier reward catalogue, including the customer's unlock and redemption state. */
export const getTierRewardVouchersForCustomer = async (
  customerId: number,
): Promise<TierVoucherRow[]> => {
...
export const redeemTierRewardVoucher = async (
  ...
  try {
    ...
    if (!voucher) {
      throw new LoyaltyVoucherRedemptionError(
        "Voucher không tồn tại, không thuộc quà theo hạng hoặc đã hết hạn.",
        404,
      );
    }
    ...
    if (MEMBER_LEVEL_RANK[customerLevel] < MEMBER_LEVEL_RANK[requiredLevel]) {
      throw new LoyaltyVoucherRedemptionError(
        "Hạng thành viên hiện tại chưa mở khóa voucher này.",
      );
    }
    ...
    if (ownedVoucherRows.length > 0) {
      throw new LoyaltyVoucherRedemptionError(
        "Bạn đã đổi voucher quà tặng của hạng này rồi.",
      );
    }
    ...
    await connection.query(
      ...
      [
        customer.id,
        pointsCost,
        `Đổi ${pointsCost} điểm nhận voucher ${voucher.code}`,
      ],
    );
    await connection.query(
...
export const getPromotions = async (): Promise<any[]> => {
  return query(
    "SELECT * FROM promotions WHERE is_active = 1 AND NOW() BETWEEN start_date AND end_date",
  );
};
...
export const getCustomerBookings = async (
  ...
  try {
    ...
    console.warn(
      "Auto-creation of booking_reviews skipped:",
  ...
  const custRows = await query<any[]>(
    "SELECT phone FROM customers WHERE id = ? AND is_deleted = 0",
    [customerId],
  );
...
export const createBookingReview = async (data: {
  ...
  if (booking.status !== "completed") {
    throw new Error(
      "Chỉ có thể đánh giá bữa ăn cho các đơn đặt bàn đã hoàn thành!",
    );
  }
  ...
  `,
    [
      data.booking_id,
      data.customer_id || booking.customer_id || null,
      ratingNum,
      data.comment || null,
      pointsAwarded,
    ],
  ...
  if (targetCustId) {
    ...
    if (custRows && custRows.length > 0) {
      ...
      await query(
        "UPDATE customers SET loyalty_points = ?, member_level = ? WHERE id = ?",
        [newPoints, memberLevel, targetCustId],
      );
      ...
      `,
        [
          targetCustId,
          pointsAwarded,
          `Thưởng +30 PTS đánh giá chất lượng bữa ăn (Mã đơn: ${booking.confirmation_code || `#${booking.id}`})`,
        ],
      );
    }
  }
...
export const getCustomerEventContracts = async (
  customerId: number | string,
): Promise<any[]> => {
  return query(
    `
    ...
    const mockAttendance =
      (
        globalThis as typeof globalThis & {
          __MOCK_ATTENDANCE?: AttendanceRecordRow[];
        }
      ).__MOCK_ATTENDANCE ?? [];
    return mockAttendance
      .filter((record) => record.employee_role !== "sales_event")
      .sort((first, second) =>
        (second.clock_out ?? second.clock_in).localeCompare(
          first.clock_out ?? first.clock_in,
        ),
      );
  }
...
export const getAttendanceEmployees = async (): Promise<
  Array<{ id: number; full_name: string; role_name: string }>
> => {
  if (!dbAvailable) {
    return MOCK_USERS.filter((user) => user.role_name !== "sales_event").map(
      (user) => ({
        id: Number(user.id),
...
export const clockInEmployee = async (
  ...
  if (!dbAvailable) {
    ...
    const newRecord = {
      ...
      clock_out: null,
      schedule_id: timing.scheduleId ?? null,
      is_late: timing.isLate ? 1 : 0,
  ...
  const result = await query(
    ...
    [
      employeeId,
      timing.scheduleId ?? null,
      timing.isLate ? 1 : 0,
      timing.lateReason ?? null,
    ],
  );
  return {
    id: result.insertId,
...
export const splitResmanagerTable = async (
  ...
  for (let i = 0; i < groups.length; i++) {
    if (
      !Number.isInteger(groups[i].guest_count) ||
      groups[i].guest_count <= 0
    ) {
      throw new TableSplitValidationError(
        `Số lượng khách nhóm ${i + 1} phải lớn hơn 0.`,
      );
    }
  }
  ...
  try {
    ...
    if (
      table.status !== TABLE_STATUS.SERVING &&
      table.status !== TABLE_STATUS.EMPTY
    ) {
      throw new TableSplitValidationError(
        `Bàn ${table.name} đang ở trạng thái '${table.status}', không thể tách bàn.`,
      );
    }
    ...
    if (totalGuests > Number(table.capacity)) {
      throw new TableSplitValidationError(
        `Tổng số khách các nhóm (${totalGuests}) vượt quá sức chứa bàn vật lý (${table.capacity} chỗ).`,
      );
    }
    ...
    if (table.merged_into_table_id !== null || merges.length > 0) {
      throw new TableSplitValidationError(
        "Không thể tách bàn đang thuộc cụm gộp. Vui lòng gỡ gộp bàn trước.",
      );
    }
    ...
    if (existingSessions.length > 0) {
      throw new TableSplitValidationError(
        `Bàn ${table.name} đang có phiên tách bàn hoạt động. Vui lòng hoàn tất phiên hiện tại trước.`,
      );
    }
...
export const getTableActiveSplits = async (tableId: number): Promise<any> => {
  ...
  for (const split of splits) {
    ...
    split.items = items;
    split.total_amount = items.reduce(
...
export const moveSplitOrderItems = async (
  ...
  if (sourceChildOrderId === targetChildOrderId) {
    throw new TableSplitValidationError(
      "Nhóm nguồn và nhóm đích phải khác nhau.",
    );
  }
  ...
  try {
    ...
    if (moveQuantity > item.quantity) {
      throw new TableSplitValidationError(
        `Số lượng chuyển (${moveQuantity}) vượt quá số lượng hiện có (${item.quantity}).`,
      );
    }
...
export const completeSubOrderPayment = async (
  childOrderId: number,
): Promise<{
  ...
  try {
    ...
    await connection.query(
      "UPDATE table_splits SET status = 'paid', closed_at = NOW() WHERE id = ?",
      [split_id],
    );
...
export const splitInvoiceByItems = async (
  ...
  try {
    ...
    const [parentInvoices] = await connection.query<any[]>(
...
export const splitInvoiceEqually = async (
  ...
  try {
    ...
    const totalAmount = Number(
      parentInvoice.final_amount || parentInvoice.subtotal,
    );
    ...
    for (let i = 0; i < guestCount; i++) {
      const childAmount =
        i === guestCount - 1 ? baseAmount + remainder : baseAmount;
...
export const updateRestaurantInfo = async (data: any): Promise<any> => {
  ...
  values.push(1);
  await query(
    `UPDATE restaurant_info SET ${fields.join(", ")} WHERE id = ?`,
    values,
  );
...
export const processOrderItemRefund = async (data: {
  ...
  const {
    orderId,
    itemIds,
    reason = "Khách yêu cầu hoàn tiền sau thanh toán",
    refundMethod = "cash",
  } = data;
