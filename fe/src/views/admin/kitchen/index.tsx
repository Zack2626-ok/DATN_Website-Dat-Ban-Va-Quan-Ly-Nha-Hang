...
export const ChefKitchenQueue: React.FC = () => {
  ...
  const handleStartCooking = (orderId: string) => {
    dispatch(updateOrderStatus({ id: orderId, status: ORDER_STATUS.IN_KITCHEN }));
  };
      ...
      <div className="flex justify-between items-center border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-4 text-xs font-semibold">
          ...
          <span className="bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700 flex items-center gap-1.5">
            <ChefHat size={14} className="text-blue-400 animate-pulse" /> Đang xử lý: {columns.waiting.length + columns.cooking.length}
          </span>
        </div>
      </div>
                          ...
                          <div key={idx} className="flex flex-col gap-1">
                            <div className="flex justify-between text-xs font-bold text-slate-200">
                              <span className="flex items-center gap-1.5">
                                ...
                                <span className={idx === 0 ? "line-through text-slate-400" : ""}>
                                  {item.name}
                                </span>
                              </span>
                              <span className="text-blue-400">x{item.quantity}</span>
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-2 mt-1">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex justify-between text-xs font-bold text-slate-400">
                            <span className="flex items-center gap-1.5">
                              <Check size={11} className="text-emerald-500" />
