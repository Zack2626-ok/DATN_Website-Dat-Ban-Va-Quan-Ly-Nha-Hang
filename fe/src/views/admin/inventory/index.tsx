...
import { restockIngredient, setIngredientStockDirect } from "../../../store/inventorySlice";
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col gap-6 animate-fade-in text-admin-text-main">
      <div className="flex justify-between items-center pb-4 border-b border-admin-border">
        <div>
          <h3 className="text-xl font-extrabold font-display">Quản lý Tồn kho</h3>
          <p className="text-xs text-admin-text-sub mt-1">Theo dõi nguyên liệu và mức tồn kho tối thiểu</p>
        </div>
      ...
      {inventory.some(ing => ing.stock <= ing.threshold) && (
        <div className="bg-rose-50 border border-rose-200 text-rose-800 rounded-xl p-4 flex items-center gap-3 text-xs font-semibold shadow-2xs">
          ...
          <span>
            <strong>Cảnh báo tồn kho thấp:</strong> {inventory.filter(ing => ing.stock <= ing.threshold).length} nguyên liệu sắp hết. Vui lòng kiểm kê và nhập kho ngay.
          </span>
        </div>
      )}
            ...
            >
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-sm text-slate-900">{ing.name}</span>
                  {isLow && (
              ...
              <div className="flex items-center gap-4">
                <span className={`text-sm font-black tracking-wider ${isLow ? "text-rose-600" : "text-admin-primary"}`}>
                  {ing.stock} {ing.unit}
                </span>
                
                <div className="flex gap-1.5">
                  <button
                    onClick={() => handleRestock(ing.id, ing.unit === "kg" ? 5 : 100)}
                    title="Nhập kho thêm"
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/60 text-xs text-slate-500 font-medium leading-relaxed text-center">
        💡 <strong>Liên kết thực đơn thực tế:</strong> Việc hạ mức tồn kho của <strong>Cá hồi</strong> hoặc <strong>Trứng cá tầm</strong> về 0 sẽ tự động ẩn và đánh dấu "Hết hàng" đối với các món ăn liên quan ở thực đơn khách hàng và nhân viên.
      </div>
    </div>
  );
};
