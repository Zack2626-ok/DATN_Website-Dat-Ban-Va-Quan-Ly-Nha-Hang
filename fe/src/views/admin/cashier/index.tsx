...
import { CreditCard, DollarSign, Wallet, Users, Info, CheckCircle2 } from "lucide-react";
...
export const CashierPOS: React.FC = () => {
  ...
  const occupiedTables = useMemo(() => {
    return tables.filter(
      (t) => t.status === TABLE_STATUS.OCCUPIED || t.status === TABLE_STATUS.CLEANING
    );
  }, [tables]);
  ...
  const [selectedTableId, setSelectedTableId] = useState<string | null>(initialSelectedTableId);
    ...
    <div className="flex flex-col gap-6 animate-fade-in text-slate-800">
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex items-center gap-4 text-emerald-900 animate-fade-in">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-700 mt-0.5">
              Đơn hàng của <strong>{lastPaidTable}</strong> đã được thanh toán hoàn tất. Bàn hiện đang chờ dọn dẹp.
            </p>
          </div>
        </div>
      )}
      ...
      {selectedTable && activeOrder ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col gap-5 shadow-2xs">
            <div className="border-b border-slate-100 pb-4">
              <h4 className="text-base font-black text-slate-900 font-display">Hóa đơn chi tiết</h4>
              ...
              <div className="flex items-center justify-between text-xs font-semibold text-slate-500">
                ...
                <div className="relative rounded-lg bg-slate-50 border border-slate-200 flex items-center px-3 py-1">
                  <input
                    ...
                    placeholder="0"
                  />
                  <span className="text-[11px] text-slate-400 ml-1">.000 vnđ</span>
                </div>
              </div>

              <div className="flex justify-between items-center border-t border-slate-100 pt-4 mt-1">
                <span className="text-sm font-black text-slate-900 font-display">Tổng cộng</span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col gap-6 shadow-2xs">
            <div className="border-b border-slate-100 pb-4">
              <h4 className="text-base font-black text-slate-900 font-display">Tách hóa đơn</h4>
              <p className="text-xs text-slate-400 mt-1">Hỗ trợ chia đều hoặc chia theo món ăn</p>
            </div>
            ...
            {splitMode === "equal" ? (
                ...
                <div className="flex flex-col gap-2 border-t border-slate-100 pt-4 max-h-[160px] overflow-y-auto pr-1">
                  {Array.from({ length: splitCount }).map((_, i) => (
                    ...
                    >
                      ...
                      <span className="text-slate-900 font-extrabold">{(perPersonAmount * 1000).toLocaleString("vi-VN")} vnđ</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-3.5 flex gap-2.5 text-xs text-blue-800 leading-relaxed font-semibold">
                  ...
                  <span>
                    Chế độ tách theo món cho phép gán riêng từng món cho từng thực khách. Chọn phương thức thanh toán bên dưới để tiếp tục thanh toán tổng hóa đơn.
                  </span>
                </div>
                  ...
                  {activeOrder.items.map((item, idx) => (
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 font-medium">Đơn giá: {(item.price * 1000).toLocaleString("vi-VN")} vnđ</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-slate-500 bg-slate-200/50 px-2 py-1 rounded">
                          Khách 1
                        </span>
                        <span className="font-bold text-slate-900">{((item.price * item.quantity) * 1000).toLocaleString("vi-VN")} vnđ</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="border-t border-slate-100 pt-5 mt-auto flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 flex items-center justify-center gap-2 cursor-pointer transition-all hover:border-slate-300"
                >
                  <CreditCard size={15} className="text-slate-500" /> Thẻ tín dụng
                </button>
                <button
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center text-slate-400">
          ...
          <h4 className="font-bold text-sm text-slate-700">Chưa chọn bàn nào để thanh toán</h4>
          <p className="text-xs mt-1">Hãy chọn một bàn đang hoạt động ở danh sách phía trên để bắt đầu lập hóa đơn.</p>
        </div>
      )}
    </div>
  );
