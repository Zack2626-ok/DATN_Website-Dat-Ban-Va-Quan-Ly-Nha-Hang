...
export const OrderTableListPage: React.FC = () => {
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-400 animate-pulse">Đang tải danh sách bàn...</div>;
  }

  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-slate-800 font-display">Gọi món</h1>
      ...
      {activeTables.length === 0 ? (
        <div className="bg-white rounded-2xl border border-sky-100 p-12 text-center">
          ...
          <p className="font-bold text-slate-500">Chưa có bàn đang phục vụ</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {activeTables.map((table) => (
            <button
              ...
              className="bg-white rounded-2xl border border-sky-100 p-5 flex items-center justify-between hover:border-blue-300 hover:shadow-md transition-all text-left group"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-black text-lg text-slate-800">{table.name}</p>
