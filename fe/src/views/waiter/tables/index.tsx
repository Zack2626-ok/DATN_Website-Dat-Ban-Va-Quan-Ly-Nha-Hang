...
export const WaiterTableMap: React.FC<WaiterTableMapProps> = ({ isManager = false }) => {
  ...
  useEffect(() => {
    ...
    if (navState?.autoOpenAssignedBooking) {
      setAssignedBookingBanner(navState.autoOpenAssignedBooking);
      setPrefilledBookingData({
        guestCount: Number(navState.autoOpenAssignedBooking.partySize || 30),
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (areasData && areasData.length > 0) {
        setSelectedAreaId((prev) => prev || areasData[0].id);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    socket.on("table:status_changed", () => {
      ...
      if (selectedTableIdRef.current) {
        loadActiveOrder(selectedTableIdRef.current);
      }
    });

    socket.on("table:booking_checked_in", () => {
      fetchData();
      if (selectedTableIdRef.current) {
        loadActiveOrder(selectedTableIdRef.current);
      }
  ...
  }, [tables, activeAssignedBooking, savedAssignedBooking]);
  ...
  const handleUnsplitTable = async () => {
    if (!selectedTable) return;
    try {
      await unsplitTable(selectedTable.id);
      toast.success(`✅ Đã hủy tách bàn ${selectedTable.name} trở về 1 bàn đơn duy nhất!`);
      fetchData();
    } catch (err: any) {
      toast.error(err?.response?.data?.message || "Không thể hủy tách bàn");
    }
  };
  ...
  const isTableWaitingArrival = useMemo(() => {
    if (!selectedTable) return false;
    const bStatus = (selectedTable as any).booking_status;
    const hasBooking = Boolean(selectedTable.booking_id);
    return hasBooking && (bStatus === "pending" || bStatus === "confirmed");
  }, [selectedTable]);
  ...
  const handleOpenTable = async (data: { guestCount: number; customerName: string; customerPhone: string }) => {
    ...
    try {
      ...
      const bId = currentAssigned?.bookingId || currentAssigned?.id || null;
      ...
      const newOrder = await createOrder({
        ...
        booking_id: bId ? Number(bId) : null,
      });
      ...
      const targetPartySize = Number(data.guestCount || currentAssigned?.partySize || 30);
      ...
      if (exceedsCap) {
        toast.error(
          `⚠️ Bàn ${selectedTable?.name} mới chỉ có sức chứa ${clusterCap}/${targetPartySize} khách. Bạn có thể nhấn "Gộp bàn" hoặc "Xếp đoàn" bên dưới để gộp đủ ${targetPartySize} chỗ!`,
          { duration: 7000 }
        );
      } else {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (t.is_split && t.status !== "empty" && t.status !== "cleaning") {
                    ...
                    >
                      ...
                      {t.is_split && t.status !== "empty" && t.status !== "cleaning" && (t.split_labels?.length ?? 0) > 0 && (
                        <span
        ...
        <div className="w-full lg:w-96 shrink-0 lg:sticky lg:top-20 space-y-4">
          {selectedTable ? (
            <div className="bg-white/80 backdrop-blur-xl border border-sky-100 shadow-md overflow-hidden animate-fade-in">
              ...
              {selectedTable.is_split && selectedTable.status !== "empty" && selectedTable.status !== "cleaning" && (
                <div className="mx-5 mt-4 flex flex-col gap-2 rounded-xl border-2 border-pink-300 bg-pink-50 p-3.5 text-xs text-pink-900 shadow-2xs">
                  ...
                  <div className="flex flex-col gap-2 mt-1">
                    <button
                      ...
                      className="w-full py-2.5 bg-pink-600 hover:bg-pink-700 text-white rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2 shadow-sm cursor-pointer"
                    >
                      <Copy size={14} />
                      Xem & Thao tác Sub-Orders ({selectedTable.split_labels?.join(", ") || "Sub-orders"})
                    </button>
                    <button
                      onClick={handleUnsplitTable}
                      className="w-full py-2 bg-white hover:bg-pink-100 text-pink-700 border border-pink-300 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <RefreshCw size={13} />
                      Hủy tách bàn (Gộp về 1 bàn đơn)
                    </button>
                  </div>
                </div>
              )}
                        ...
                        <div className="border-t border-sky-50 pt-4 space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            {selectedTable.status === "serving" && !selectedTable.is_merged_child && (
                              isTableWaitingArrival ? (
                                <button
                                  onClick={() => {
                                    toast("ℹ️ Bàn này đã xếp trước cho đoàn. Vui lòng vào Lịch đặt bàn bấm \"Mở bàn\" khi khách đến để gọi món!", { icon: "🔒", duration: 5000 });
                                  }}
                                  className="flex items-center gap-1 rounded-lg bg-amber-100 text-amber-900 border border-amber-300 px-2.5 py-1 text-[11px] font-bold cursor-pointer hover:bg-amber-200 shadow-2xs transition-colors"
                                  title="Bàn đã xếp trước. Vào Lịch đặt bàn bấm Mở bàn khi khách đến!"
                                >
                                  <Lock size={13} className="text-amber-700" /> Khóa gọi món (Chờ khách đến)
                                </button>
                              ) : isTableOverClusterCapacity(selectedTable) ? (
                                <button
                                  onClick={() => {
                          ...
                          {/* TỔNG TIỀN VÀ IN PHIẾU TẠM TÍNH — chỉ hiển thị khi có món và khách đã đến */}
                          {activeOrder && !isTableWaitingArrival && activeOrder.items.filter(i => i.status !== "voided" && i.status !== "cancelled").length > 0 && (
                          ...
                          {selectedTable.status === "serving" && !isTableWaitingArrival && !selectedTable.is_early_paid && !selectedTable.is_early_payment && activeOrder && activeOrder.items.filter(i => i.status !== "voided" && i.status !== "cancelled").length > 0 && (
                            <button
